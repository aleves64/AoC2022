...
def travel_from(initial_state, end):
    ...
    return state, dist[state]

blizzard_states = get_blizzard_states()
start = (grid[0].index("."),0)
end = (grid[-1].index("."),height-1)
at_entrance = (start, 0)
at_goal, dist1 = travel_from(at_entrance, end)
at_entrance1, dist2 = travel_from(at_goal, start)
at_goal1, dist3 = travel_from(at_entrance1, end)
